...
type Draft = { firstName:string;lastName:string;phone:string;alternatePhone:string;email:string;address:string;municipality:string;province:string;postcode:string;reference:string;notes:string;deliveryWindow:string;paymentMethod:string };
const emptyDraft:Draft={firstName:"",lastName:"",phone:"",alternatePhone:"",email:"",address:"",municipality:"",province:"",postcode:"",reference:"",notes:"",deliveryWindow:"",paymentMethod:""};
...
export default function CheckoutClient({initialReferral}:{initialReferral:string}){
  ...
  return <div className="checkout-shell">
    <form className="checkout-form" onSubmit={submit} noValidate={false}>
      ...
      <fieldset disabled={busy}><legend>Entrega</legend><label><span>Dirección completa</span><input autoComplete="street-address" required minLength={8} value={draft.address} onChange={e=>field("address",e.target.value)}/></label><div className="field-grid two"><label><span>Municipio o zona</span><input autoComplete="address-level2" required minLength={2} value={draft.municipality} onChange={e=>field("municipality",e.target.value)}/></label><label><span>Provincia</span><input autoComplete="address-level1" required minLength={2} value={draft.province} onChange={e=>field("province",e.target.value)}/></label></div><div className="field-grid two"><label><span>Código postal</span><input inputMode="numeric" autoComplete="postal-code" required minLength={3} maxLength={12} value={draft.postcode} onChange={e=>field("postcode",e.target.value)}/></label><label><span>Franja preferida <small>Opcional</small></span><select value={draft.deliveryWindow} onChange={e=>field("deliveryWindow",e.target.value)}><option value="">Sin preferencia</option><option value="Mañana">Mañana</option><option value="Tarde">Tarde</option></select></label></div><label><span>Referencia para llegar <small>Opcional</small></span><input value={draft.reference} onChange={e=>field("reference",e.target.value)}/></label><label><span>Notas <small>Opcional</small></span><textarea rows={3} maxLength={500} value={draft.notes} onChange={e=>field("notes",e.target.value)}/></label></fieldset>
