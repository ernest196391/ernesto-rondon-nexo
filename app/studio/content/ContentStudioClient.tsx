...
export default function ContentStudioClient() {
  ...
  function resetResult() { if (resultUrl) URL.revokeObjectURL(resultUrl); setResultUrl(""); }
  ...
  async function renderApproved() {
    ...
    body.append("words", JSON.stringify(transcript.words));
    body.append("cuts", JSON.stringify(transcript.cutPlan ?? []));
    try {
      const response = await fetch("/api/studio/content/render", { method: "POST", body });
      if (!response.ok) {
        const data = (await response.json().catch(() => ({}))) as { error?: string };
        throw new Error(data.error || "No se pudo renderizar el plan aprobado.");
      }
  ...
  return <div className="content-app">
    <form onSubmit={submit} className="content-form">
      ...
      <button disabled={loading || !file} type="submit">{loading ? "Procesando…" : "Versión vertical rápida"}</button>
      <small>La transcripción usa la API configurada y solo genera consumo cuando pulsas transcribir.</small>
    </form>
    ...
    {transcript && <section className="content-result" aria-live="polite">
      ...
      <div><p><strong>Idioma:</strong> {transcript.language || "no informado"} · <strong>Duración:</strong> {transcript.durationSeconds ? `${transcript.durationSeconds.toFixed(1)} s` : "—"}</p><p>{transcript.text}</p><h3>Plan de cortes por pausas</h3>{transcript.cutPlan?.length ? transcript.cutPlan.map((cut, index) => <article key={`${cut.start}-${cut.end}`}><strong>Corte {index + 1}: {cut.start.toFixed(1)}–{cut.end.toFixed(1)} s</strong><p>{cut.reason}</p></article>) : <p>No se detectaron pausas suficientes; se conservará la intervención completa.</p>}
        <label><input type="checkbox" checked={approved} onChange={(event) => setApproved(event.target.checked)} /> Confirmo que NEXO entendió la transcripción y el plan.</label>
        <button type="button" disabled={!approved || rendering || !transcript.words?.length} onClick={renderApproved}>{rendering ? "Renderizando edición…" : "4. Renderizar cortes + subtítulos sincronizados"}</button>
      </div>
    </section>}
