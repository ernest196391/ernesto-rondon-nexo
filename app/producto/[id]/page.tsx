...
export default async function ProductPage({ params, searchParams }: {
  ...
  return <main className="product-page">
    ...
    <div className="product-layout"><section className="product-gallery">{imageSrc ? <img src={imageSrc} alt={product.images?.[0]?.alt || product.name} /> : <div>Sin fotografía</div>}</section>
      <section className="product-purchase"><small>{product.categories?.map((x: any) => x.name).join(" · ")}</small><h1>{product.name}</h1><strong className="product-price">{publicPrice} USD</strong><div dangerouslySetInnerHTML={{ __html: content.shortDescriptionHtml }} />
