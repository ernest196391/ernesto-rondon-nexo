...
export default function ImpulsaClient({initial}:{initial:Dashboard}){
 ...
 const margin=activeRule?.mode==="fixed"?`+${activeRule.value} USD`:activeRule?.mode==="percent"?`+${activeRule.value}%`:"Precio NEXO";
 ...
 async function share(){try{if(navigator.share)await navigator.share({title:`Tienda de ${data.profile.publicName}`,text:"Mira mi selección en NEXO",url:storeUrl});else{await navigator.clipboard.writeText(storeUrl);setMessage("Enlace copiado.")}}catch{}}
 ...
 return <main className="impulsa-shell">
  ...
  <div className="office-wrap">
   <section id="inicio" className="impulsa-hero"><div><span>MI NEGOCIO</span><h1>Hola, {data.profile.publicName}.</h1><p>Tu espacio para vender con NEXO.</p></div><div className="store-status"><span className={data.productIds.length?"live":"draft"}>{data.productIds.length?"Publicada":"Borrador"}</span><b>{data.productIds.length} productos</b></div></section>
   <section className="office-tools simple-office" aria-label="Acciones principales">
    <a className="office-tool office-tool-primary" href="/impulsa/buscar"><span>01</span><div><b>Buscar para un cliente</b><small>Sube una foto o escribe lo que necesita. NEXO te ayuda a encontrar una oportunidad.</small></div><strong>→</strong></a>
   ...
   <section className="store-command" aria-label="Tu tienda"><div><span>TU TIENDA</span><h2>Lista para compartir</h2><p>Lo que compartas conserva tu referencia automáticamente.</p></div><div className="store-command-actions"><a href={`/g/${data.profile.slug}`}>Abrir tienda</a><button type="button" onClick={share}>Compartir mi tienda</button></div></section>
   <details id="mi-tienda" className="impulsa-panel work-detail"><summary><span><b>Administrar mi tienda</b><small>Elegir productos y cambiar precios</small></span><strong>Configurar</strong></summary><div className="work-detail-body">
    <section className="products-panel"><div className="panel-heading"><div><span>PRODUCTOS</span><h2>Qué quieres vender</h2><p>Solo se publican los que selecciones.</p></div><b>{selected.length} elegidos</b></div>
     <form className="catalog-form" onSubmit={saveProducts}><div className="catalog-tools"><label className="search-field"><span>Buscar en el catálogo</span><input type="search" value={query} onChange={e=>{setQuery(e.target.value);setShowCount(12)}} placeholder="Nombre del producto"/></label><div><button type="button" className="quiet" onClick={()=>setSelected(Array.from(new Set([...selected,...visible.map(p=>p.id)])))}>Elegir visibles</button><button type="button" className="quiet" onClick={()=>setSelected([])}>Limpiar</button></div></div><div className="impulsa-products">{visible.slice(0,showCount).map(p=><label className={selected.includes(p.id)?"selected":""} key={p.id}><input type="checkbox" checked={selected.includes(p.id)} onChange={()=>toggle(p.id)}/>{p.image&&<img src={p.image} alt=""/>}<span><b>{p.name}</b><small>{p.price} {p.currency}</small></span></label>)}</div>{visible.length>showCount&&<button type="button" className="catalog-more" onClick={()=>setShowCount(x=>x+12)}>Mostrar más</button>}{dirty&&<div className="sticky-save"><button disabled={busy}>{busy?"Guardando…":"Publicar cambios"}</button></div>}</form>
    </section>
    <section id="negocio" className="margin-inside"><div className="panel-heading"><div><span>PRECIO</span><h2>Tu margen</h2><p>Opcional. La entrega se cobra aparte.</p></div></div><form className="price-form" onSubmit={savePrice}><label>Regla<select value={mode} onChange={e=>setMode(e.target.value as "base"|"fixed"|"percent")}><option value="base">Precio NEXO</option><option value="fixed">Sumar importe</option><option value="percent">Sumar porcentaje</option></select></label>{mode!=="base"&&<label>{mode==="fixed"?"Importe (USD)":"Porcentaje"}<input type="number" min="0" step={mode==="fixed"?".01":".1"} value={value} onChange={e=>setValue(Number(e.target.value))}/></label>}<button disabled={busy}>Guardar</button></form>{sample&&<div className="price-preview"><span><small>Ejemplo</small><b>{sample.name}</b></span><span><small>NEXO</small><b>{base.toFixed(2)} USD</b></span><span><small>Tu precio</small><b>{final.toFixed(2)} USD</b></span><span><small>Ganancia</small><b>{Math.max(0,final-base).toFixed(2)} USD</b></span></div>}</section>
   </div></details>
