...
export async function POST(request: Request) {
  ...
  try {
    const response = await fetch(`${workerUrl.replace(/\/$/, "")}/analyze`, { method: "POST", body: outgoing, headers: { "x-nexo-worker-key": workerSecret }, signal: AbortSignal.timeout(70_000) });
    const data = await response.json().catch(() => null);
    if (!response.ok || !data) return NextResponse.json({ error: "El worker no pudo analizar el vídeo." }, { status: 502 });
