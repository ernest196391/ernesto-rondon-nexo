...
export async function POST(request: Request) {
  ...
  const incoming = await request.formData(); const file = incoming.get("file"); const rawWords = incoming.get("words"); const rawCuts = incoming.get("cuts"); const approved = incoming.get("approved") === "true";
  ...
  try { words = JSON.parse(typeof rawWords === "string" ? rawWords : "[]") as Word[]; cuts = JSON.parse(typeof rawCuts === "string" ? rawCuts : "[]") as Cut[]; }
  catch { return NextResponse.json({ error: "El plan de edición no es válido." }, { status: 400 }); }
  if (words.length > 5000 || cuts.length > 60) return NextResponse.json({ error: "El plan excede los límites permitidos." }, { status: 400 });
  const outgoing = new FormData(); outgoing.append("file", file, file.name); outgoing.append("plan", JSON.stringify({ approved: true, words, cuts }));
  try {
    const response = await fetch(`${workerUrl.replace(/\/$/, "")}/render`, { method: "POST", body: outgoing, headers: { "x-nexo-worker-key": workerSecret }, signal: AbortSignal.timeout(250_000) });
    if (!response.ok) { const detail = await response.text(); return NextResponse.json({ error: "El worker no pudo renderizar el plan aprobado.", detail: detail.slice(0, 1400) }, { status: 502 }); }
  ...
  } catch { return NextResponse.json({ error: "No se pudo conectar con el worker de edición." }, { status: 502 }); }
}
