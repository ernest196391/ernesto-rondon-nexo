...
final class NEXO_Marketplace_Plugin {
	const VERSION='0.2.0';
	public static function boot():void{add_action('wp_enqueue_scripts',[__CLASS__,'assets'],30);add_action('woocommerce_single_product_summary',[__CLASS__,'availability'],22);add_action('woocommerce_checkout_create_order',[__CLASS__,'order_meta'],35,2);add_action('woocommerce_checkout_order_created',[__CLASS__,'prevent_parallel_cvd_ledger'],999);add_action('woocommerce_store_api_checkout_order_processed',[__CLASS__,'prevent_parallel_cvd_ledger'],999);add_filter('woocommerce_get_breadcrumb',[__CLASS__,'breadcrumb']);add_action('admin_notices',[__CLASS__,'compatibility_notice']);}
	...
	public static function order_meta(WC_Order $order,array $data):void{unset($data);$order->update_meta_data('_nexo_marketplace_order','yes');$order->update_meta_data('_nexo_marketplace_version',self::VERSION);$order->update_meta_data('_nexo_ledger_owner','nexo');if(!$order->get_meta('_cvd_owner_user_id',true))$order->update_meta_data('_nexo_attribution_fallback','organic');}
	/** NEXO owns the economic ledger of NEXO orders. CVD metadata remains compatibility-only and never payable. */
	public static function prevent_parallel_cvd_ledger($order_or_id):void{$order=$order_or_id instanceof WC_Order?$order_or_id:wc_get_order((int)$order_or_id);if(!$order||'yes'!==$order->get_meta('_nexo_marketplace_order',true))return;$order->update_meta_data('_nexo_ledger_owner','nexo');$order->update_meta_data('_cvd_commission_status','cancelled');$order->update_meta_data('_cvd_commission_amount','0');$order->update_meta_data('_cvd_payout_status','blocked_nexo_ledger');$order->save();}
