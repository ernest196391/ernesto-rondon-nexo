...
export function searchProducts(products: any[], query: string, origin: string, ref = "", limit = 3) {
  ...
  return products.filter((product) => product.stock_status !== "outofstock" && product.catalog_visibility !== "hidden")
    .map((product) => ({ product, score: words.reduce((score, word) => `${product.name} ${product.sku || ""} ${product.categories?.map((c: any) => c.name).join(" ") || ""}`.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().includes(word) ? score + 1 : score, 0) }))
