...
describe("familias públicas", () => {
  it("mantiene exactamente seis familias ordenadas", () => {
    expect(STOREFRONT_CATEGORIES.filter((family) => family.enabled).map((family) => family.id)).toEqual(["electrodomesticos", "cocina", "habitacion", "energia", "tecnologia", "otros"]);
  });
