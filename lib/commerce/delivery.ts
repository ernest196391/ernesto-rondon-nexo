...
export type ShippingQuote={status:"zone"|"pending"|"pickup";feeCup:number;label:string;referenceCup?:number;version:string};
...
export function deliveryCatalog(){const localities:Record<string,string[]>={};for(const row of rates){if(row.active&&row.zone)(localities[row.municipality]??=[]).push(row.zone);}for(const key of Object.keys(localities))localities[key]=[...new Set(localities[key])];return {province:{code:"LH",name:"La Habana"},municipalities:Object.keys(localities),localities,rateVersion:SHIPPING_RATE_VERSION,pickup:{name:process.env.NEXO_PICKUP_NAME||"NEXO · punto de recogida",address:process.env.NEXO_PICKUP_ADDRESS||"Nuevo Vedado, La Habana",instructions:process.env.NEXO_PICKUP_INSTRUCTIONS||"Confirmaremos por WhatsApp cuándo tu pedido esté listo para recoger."}};}
export function quoteShipping(municipality:string,zone:string,manual=false):ShippingQuote{if(manual)return {status:"pending",feeCup:0,label:zone||municipality,version:SHIPPING_RATE_VERSION};const m=normalizeText(municipality),z=normalizeText(zone);let reference=0;for(const row of rates){if(!row.active||normalizeText(row.municipality)!==m)continue;if(!row.zone){reference=row.fee;continue;}if(z&&normalizeText(row.zone)===z)return {status:"zone",feeCup:row.fee,label:row.zone,version:SHIPPING_RATE_VERSION};}return {status:"pending",feeCup:0,label:zone||municipality,referenceCup:reference,version:SHIPPING_RATE_VERSION};}
