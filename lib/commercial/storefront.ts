...
export async function projectCommercialCart(cart:any,ref:string){const gestora=await getGestoraByRef(ref);if(!gestora||gestora.status!=="active")return{cart,gestora:null,lines:[] as CommercialLineSnapshot[]};const rules=await listRules(gestora.id),lines:CommercialLineSnapshot[]=[];let totalItems=0;const items=(cart.items||[]).map((item:any)=>{const minor=Number(item.prices?.currency_minor_unit??cart.totals?.currency_minor_unit??2),factor=10**minor,base=Number(item.prices?.price??0)/factor,productId=Number(item.parent_id||item.id),variationId=item.parent_id?Number(item.id):0,resolved=resolveCommercialPrice({base,currency:item.prices?.currency_code||gestora.defaultCurrency,rule:choosePriceRule(rules,productId)}),quantity=Number(item.quantity||1),line=Math.round(resolved.final*quantity*factor);totalItems+=line;lines.push({productId,variationId,quantity,baseUnit:resolved.base,markupUnit:resolved.markup,finalUnit:resolved.final,currency:resolved.currency,ruleId:resolved.ruleId,ruleVersion:resolved.ruleVersion});return{...item,prices:{...item.prices,price:String(Math.round(resolved.final*factor)),regular_price:String(Math.round(resolved.final*factor)),sale_price:String(Math.round(resolved.final*factor))},totals:{...item.totals,line_subtotal:String(line),line_total:String(line)}};});const totals={...cart.totals,total_items:String(totalItems),total_items_tax:"0",total_price:String(totalItems+Number(cart.totals?.total_shipping||0))};return{cart:{...cart,items,totals},gestora,lines};}
