...
export async function adminControlData(){await ensureAdminSchema();const [gestoras,ledger,payouts,recon,settings,campaigns,snapshots]=await Promise.all([
 db().query(`SELECT g.id,g.user_id AS "userId",g.public_name AS "publicName",g.slug,g.referral_code AS "referralCode",g.whatsapp,g.status,g.created_at AS "createdAt",COALESCE(s.sales,0)::int AS sales,COALESCE(l.available,0)::float AS "availableCommission" FROM nexo_gestora_profiles g LEFT JOIN (SELECT gestora_id,COUNT(DISTINCT woocommerce_order_id) AS sales FROM nexo_order_commercial_snapshots GROUP BY gestora_id) s ON s.gestora_id=g.id LEFT JOIN (SELECT gestora_id,SUM(amount) FILTER(WHERE status='available') AS available FROM nexo_commission_ledger GROUP BY gestora_id) l ON l.gestora_id=g.id ORDER BY g.created_at DESC`),
...
export async function adminSetting(key:string,fallback=""){await ensureAdminSchema();const r=await db().query(`SELECT value FROM nexo_admin_settings WHERE key=$1`,[key]);return String(r.rows[0]?.value??fallback);}
